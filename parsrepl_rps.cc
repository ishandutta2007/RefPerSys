...
Rps_TokenSource::parse_expression(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression start token_deq:" << token_deq << " startpos:" << startpos);
  bool ok = false;
  _f.lextokv =  lookahead_token(&_, token_deq, 0);
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression lextokv=" << _f.lextokv << " position:" << position_str() << " token_deq:" << token_deq);
  if (!_f.lextokv)
    {
      if (pokparse)
        *pokparse = false;
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression failing " << startpos);
      return nullptr;
    }
  _f.leftv = parse_disjunction(&_, token_deq, &ok);
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression left disjunction leftv=" << _f.leftv << ' '
                << (ok?"left OK":"left BAD")
                << " position:" << position_str() << " token_deq:" << token_deq);
  if (!ok)
    {
      if (pokparse)
        *pokparse = false;
  ...
  do
    {
      ...
      again = false;
      _f.lextokv =  lookahead_token(&_, token_deq, 0);
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression after left lextokv=" << _f.lextokv << " token_deq:" << token_deq << " nbloop=" << nbloop);
      if (!_f.lextokv)
        {
          again = false;
          break;
      ...
      if (_f.lextokv.is_lextoken()
        ...
        {
          (void) RPS_GET_TOKEN(*this,&_,&token_deq); // consume the or operator
          again = true;
          if (!_f.oroperob)
      ...
      if (again)
        {
          bool okright=false;
          RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression before parse_disjunction1 lextokv=" << _f.lextokv << " position:" << position_str()
                        << " token_deq:" << token_deq);
          ...
          if (okright)
            ...
            {
              RPS_WARNOUT("expression parser failed to parse disjunct at " << position_str());
              return nullptr;
            }
        }
    }
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_expression returns "
                << _f.disjv << " token_deq:" << token_deq
                << " startpos:"<< startpos << " endingpos:" << position_str());
  return _f.disjv;
...
Rps_TokenSource::parse_disjunction(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  std::string startpos = position_str();
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_disjunction start token_deq:" << token_deq << " startpos:" << startpos);
  bool ok = false;
  _f.lextokv =  lookahead_token(&_, token_deq, 0);
  ...
  do
    {
      ...
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_disjunction testing and lextokv=" << _f.lextokv << " position:" << position_str() << " token_deq:" << token_deq);
      if (_f.lextokv.is_lextoken()
          && _f.lextokv.to_lextoken()->lxkind() == RPS_ROOT_OB(_2wdmxJecnFZ02VGGFK) //repl_delimiter∈class
          &&  _f.lextokv.to_lextoken()->lxval().is_object()
          &&  _f.lextokv.to_lextoken()->lxval().to_object()->oid() == idanddelim)
        {
          (void) RPS_GET_TOKEN(*this,&_,&token_deq); // consume the and operator
          again = true;
          if (!_f.andoperob)
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_disjunction andoperob=" << _f.andoperob
                << "nbconj:" << conjvect.size() << " at " << startpos << " token_deq:" << token_deq);
  if (conjvect.size() > 1)
    {
      /// we make an instance:
...
Rps_TokenSource::parse_conjunction(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  std::string startpos = position_str();
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_conjunction start token_deq:" << token_deq << " startpos:" << startpos);
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_conjunction lextokv="
                ...
                << " andbinop:" << _f.andbinopob << " token_deq:" << token_deq);
  ...
  do
    {
      ...
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_conjunction testing or lextokv=" << _f.lextokv << " position:" << position_str() << " token_deq:" << token_deq);
      if (_f.lextokv.is_lextoken()
          && _f.lextokv.to_lextoken()->lxkind() == RPS_ROOT_OB(_2wdmxJecnFZ02VGGFK) //repl_delimiter∈class
          &&  _f.lextokv.to_lextoken()->lxval().is_object()
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_conjunction gives "
                << _f.conjv << " at " << startpos << " token_deq:" << token_deq);
  return _f.conjv;
...
Rps_TokenSource::parse_comparison(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  std::string startpos = position_str();
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_comparison start token_deq:" << token_deq << " startpos:" << startpos);
  /// lessequal is <=
  ...
  if (okleft)
    {
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_comparison got leftv=" << _f.leftv
...
Rps_TokenSource::parse_comparand(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  if (okleft)
    {
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_comparand leftv=" << _f.leftv << " startpos:" << startpos
                    << " currentpos:" << position_str() << " token_deq:" << token_deq);
    }
  else
    {
  ...
  if (!_f.lexopertokv)
    {
      (void) RPS_GET_TOKEN(*this,&_,&token_deq); // consume the token
      if (pokparse)
        *pokparse = true;
      RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_comparand gives leftv:" << _f.leftv << " tokendeq:" << token_deq
                    << std::endl
                    << RPS_FULL_BACKTRACE_HERE(1, "Rps_TokenSource::parse_comparand/left"));
      return _f.leftv;
    }
...
Rps_TokenSource::parse_factor(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  std::string startpos = position_str();
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_factor start token_deq:" << token_deq << " startpos:" << startpos);
  ...
  if (_f.binoperob)
    {
      ...
      _f.lexgotokv = RPS_GET_TOKEN(*this,&_,&token_deq);
...
Rps_TokenSource::parse_term(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  std::string startpos = position_str();
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term start startpos:" << startpos << " token_deq:" << token_deq);
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term  startpos:" << startpos << " multdelimob:" << _f.multdelimob
                << " multbinopob: " << _f.multbinopob << " token_deq:" << token_deq);
          ...
          if (_f.lexoperdelimob == _f.multdelimob)
          ...
          else
            {
              RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term lexopertokv:" << _f.lexopertokv
                            << " strange lexoperdelimob:" << _f.lexoperdelimob << " :!-> return leftv:" << _f.leftv << " token_deq:" << token_deq);
              if (pokparse)
                *pokparse = true;
              return _f.leftv;
            }
          ...
          if (_f.binoperob == _f.curoperob)
            {
              ...
              RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term operandvect:" << operandvect << " leftv=" << _f.leftv
                            << " before parse_primary of right" << position_str() << " token_deq:" << token_deq);
              _f.rightv = parse_primary(&_, token_deq, &okright);
              if (!okright)
                {
              ...
              RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term operandvect:" << operandvect << " leftv=" << _f.leftv
                            ...
                            << " " << Rps_QuotedC_String(curcptr()) << " token_deq:" << token_deq);
              again = true;
              continue;
            }
          else
...
#warning Rps_TokenSource::parse_term make two things?
      ...
      else
        {
          RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_term breakingloop operandvect:" << operandvect << " leftv=" << _f.leftv
                        ...
                        << " binoperob=" << _f.binoperob << " token_deq:" << token_deq);
          break;
        };
    } // end while (again)
...
Rps_TokenSource::parse_primary(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, bool*pokparse)
{
  ...
  std::string startpos = position_str();
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_primary start startpos:" << startpos << " token_deq:" << token_deq);
  _f.lextokv =  lookahead_token(&_, token_deq, 0);
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_primary lextokv=" << _f.lextokv << " position:" << startpos << " token_deq:" << token_deq);
  if (!_f.lextokv)
    {
      if (pokparse)
        *pokparse = false;
      ...
      if (_f.lexvalv.is_string())
        {
          ...
          if (!_f.symbob)
          ...
          else
            {
              if (pokparse)
                *pokparse = true;
              (void) RPS_GET_TOKEN(*this, &_, &token_deq);
              return _f.lexvalv;
            }
        }
      else
...
Rps_TokenSource::parse_primary_complement(Rps_CallFrame*callframe, std::deque<Rps_Value>& token_deq, Rps_Value primaryexparg, bool*pokparse)
{
  ...
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_primary_complement start startpos:" << startpos << " token_deq:" << token_deq << " primaryexpv:" << _f.primaryexpv);
  _f.lextokv =  lookahead_token(&_, token_deq, 0);
  RPS_DEBUG_LOG(REPL, "Rps_TokenSource::parse_primary_complement lextokv=" << _f.lextokv
                << " token_deq:" << token_deq
